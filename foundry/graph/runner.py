...
from foundry.tools.diff_reader import write_empty_diff
...
def run_cycle(foundry_home: Path, artifact_home: Path, *, project_id: str, task: str) -> CycleArtifacts:
    task_id = make_task_id()
    run_dir = artifact_home / "runs" / task_id
    event_log = EventLog(run_dir / "events.jsonl")
    event_log.append("cycle_started", {"project_id": project_id, "task": task})
    ...
    build = CodexRunner(foundry_home, allow_real_codex=False).run_builder(
        task_id=task_id,
        request_id=work_request.request_id,
        workspace_path=workspace.repo_path,
        prompt=task,
    )
    event_log.append("mock_builder_result", build.model_dump(mode="json"))

    diff_path = run_dir / "diff.patch"
    write_empty_diff(diff_path)
    risk = SafetyChecker().scan(
        task_id=task_id,
        actor_agent="orchestra",
    ...
    if not workspace.source_exists:
        decision = "human_review"
        summary = (
        ...
        decision = "repair"
        summary = "Mock cycle produced findings that require repair before acceptance."
